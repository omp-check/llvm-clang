...
StmtResult Parser::ParseTSSDeclarativeOrExecutableDirective(
                                                   bool StandAloneAllowed) {
  assert(Tok.is(tok::annot_pragma_tss) && "Not an TSS directive!");
  ...
  case OMPD_threadprivate:
    ...
    if (!ParseOpenMPSimpleVarList(OMPD_threadprivate, Identifiers, false)) {
      ...
      // extra tokens.
      if (Tok.isNot(tok::annot_pragma_tss_end)) {
        Diag(Tok, diag::warn_omp_extra_tokens_at_eol)
          << getOpenMPDirectiveName(OMPD_threadprivate);
        SkipUntil(tok::annot_pragma_tss_end, false, true);
      }
      DeclGroupPtrTy Res =
    ...
    SkipUntil(tok::annot_pragma_tss_end, false);
    break;
  case OMPD_critical:
  ...
  case OMPD_ordered: {
    ...
    while (Tok.isNot(tok::annot_pragma_tss_end)) {
    ...
    if (DKind == OMPD_parallel_for || DKind == OMPD_parallel_sections) {
      ...
      Toks[0].startToken();
      Toks[0].setKind(tok::annot_pragma_tss);
