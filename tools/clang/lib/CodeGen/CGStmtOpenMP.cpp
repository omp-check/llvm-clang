...
void CodeGenFunction::EmitOMPParallelDirective(const OMPParallelDirective &S) {
  ...
  llvm::Function *Fn = llvm::Function::Create(getTypes().GetFunctionType(FI),
	...
	if(f_log)
		...
		f = fopen("temp_check_functions.log", "w");
		f_log = true;
	}
...
void CodeGenFunction::EmitOMPForDirective(const OMPForDirective &S) {
  ...
  // CodeGen for "omp for {Associated statement}".
  {
    ...
    const Expr *IterVar = S.getNewIterVar();
    int TypeSize = 4;
    ...
    llvm::Value *UB = EmitScalarExpr(S.getNewIterEnd());
#ifdef DEBUG
    llvm::AllocaInst *DebugUB = CreateMemTemp(S.getNewIterEnd()->getType(), "debug.ub");
    ...
    llvm::BasicBlock *OMPLoopBB = createBasicBlock("omp.loop.begin");
    ...
    QualType QTy = S.getNewIterVar()->getType();
    ...
    llvm::BasicBlock *FiniBB = createBasicBlock("omp.loop.fini");

    ArrayRef<Expr *> Arr = S.getCounters();
//    llvm::SmallVector<const Expr *, 16> Incs;
    ...
    for (unsigned I = 0; I < S.getCollapsedNumber(); ++I) {
...
void CodeGenFunction::EmitInitOMPClause(const OMPClause &C,
  ...
  case OMPC_nowait:
    ...
    break;
  case OMPC_check:
    EmitInitOMPCheckClause(cast<OMPCheckClause>(C), S);
    break;
  case OMPC_ordered:
...
void CodeGenFunction::EmitInitOMPCheckClause(const OMPCheckClause &C,
                                              const OMPExecutableDirective &S) {
  CGM.OpenMPSupport.setCheck(true);
}
